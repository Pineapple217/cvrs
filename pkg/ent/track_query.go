...
func (tq *TrackQuery) QueryAppearingArtists() *ArtistQuery {
...
// QueryAppearance chains the current query on the "appearance" edge.
func (tq *TrackQuery) QueryAppearance() *TrackAppearanceQuery {
	query := (&TrackAppearanceClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TrackQuery) FirstID(ctx context.Context) (id pid.ID, err error) {
	var ids []pid.ID
...
func (tq *TrackQuery) FirstIDX(ctx context.Context) pid.ID {
...
func (tq *TrackQuery) OnlyID(ctx context.Context) (id pid.ID, err error) {
	var ids []pid.ID
...
func (tq *TrackQuery) OnlyIDX(ctx context.Context) pid.ID {
...
func (tq *TrackQuery) IDs(ctx context.Context) (ids []pid.ID, err error) {
...
func (tq *TrackQuery) IDsX(ctx context.Context) []pid.ID {
	ids, err := tq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (tq *TrackQuery) WithAppearingArtists(opts ...func(*ArtistQuery)) *TrackQuery {
	query := (&ArtistClient{config: tq.config}).Query()
...
// WithAppearance tells the query-builder to eager-load the nodes that are connected to
// the "appearance" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TrackQuery) WithAppearance(opts ...func(*TrackAppearanceQuery)) *TrackQuery {
	query := (&TrackAppearanceClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withAppearance = query
	return tq
}
...
func (tq *TrackQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Track, error) {
	...
	if query := tq.withAppearingArtists; query != nil {
	...
	if query := tq.withAppearance; query != nil {
		if err := tq.loadAppearance(ctx, query, nodes,
			func(n *Track) { n.Edges.Appearance = []*TrackAppearance{} },
			func(n *Track, e *TrackAppearance) { n.Edges.Appearance = append(n.Edges.Appearance, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TrackQuery) loadAppearingArtists(ctx context.Context, query *ArtistQuery, nodes []*Track, init func(*Track), assign func(*Track, *Artist)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[pid.ID]*Track)
	nids := make(map[pid.ID]map[*Track]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(track.AppearingArtistsTable)
		s.Join(joinT).On(s.C(artist.FieldID), joinT.C(track.AppearingArtistsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(track.AppearingArtistsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(track.AppearingArtistsPrimaryKey[1]))
...
func (tq *TrackQuery) loadAppearance(ctx context.Context, query *TrackAppearanceQuery, nodes []*Track, init func(*Track), assign func(*Track, *TrackAppearance)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(trackappearance.FieldTrackID)
	}
	query.Where(predicate.TrackAppearance(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(track.AppearanceColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "track_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
