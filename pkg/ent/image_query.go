...
func (iq *ImageQuery) QueryRelease() *ReleaseQuery {
	query := (&ReleaseClient{config: iq.config}).Query()
...
func (iq *ImageQuery) FirstID(ctx context.Context) (id pid.ID, err error) {
	var ids []pid.ID
...
func (iq *ImageQuery) FirstIDX(ctx context.Context) pid.ID {
...
func (iq *ImageQuery) OnlyID(ctx context.Context) (id pid.ID, err error) {
	var ids []pid.ID
...
func (iq *ImageQuery) OnlyIDX(ctx context.Context) pid.ID {
...
func (iq *ImageQuery) IDs(ctx context.Context) (ids []pid.ID, err error) {
...
func (iq *ImageQuery) IDsX(ctx context.Context) []pid.ID {
	ids, err := iq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (iq *ImageQuery) WithRelease(opts ...func(*ReleaseQuery)) *ImageQuery {
	query := (&ReleaseClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withRelease = query
	return iq
}
...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	...
	if query := iq.withRelease; query != nil {
		if err := iq.loadRelease(ctx, query, nodes, nil,
			func(n *Image, e *Release) { n.Edges.Release = e }); err != nil {
			return nil, err
		}
	}
...
func (iq *ImageQuery) loadRelease(ctx context.Context, query *ReleaseQuery, nodes []*Image, init func(*Image), assign func(*Image, *Release)) error {
	ids := make([]pid.ID, 0, len(nodes))
	nodeids := make(map[pid.ID][]*Image)
	for i := range nodes {
		if nodes[i].release_image == nil {
			continue
		}
		fk := *nodes[i].release_image
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "release_image" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
